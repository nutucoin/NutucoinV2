...
const std::string strMessageMagic = "Nutucoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NTU additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    if ((chainActive.Height() + 1) == 0) return true;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    if ((chainActive.Height() + 1) == 0) return;
...
void ThreadScriptCheck() {
    RenameThread("nutucoin-scriptch");
