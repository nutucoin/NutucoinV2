...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-fNotUseChangeAddress=<n>", "Reuse the old address instead of creating a new address (default: 1)", false, OptionsCategory::OPTIONS);
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if nutucoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if nutucoin is started while in a temporary directory.\n",
