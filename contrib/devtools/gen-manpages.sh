...
LITECOIND=${BITCOIND:-$BINDIR/nutucoind}
LITECOINCLI=${BITCOINCLI:-$BINDIR/nutucoin-cli}
LITECOINTX=${BITCOINTX:-$BINDIR/nutucoin-tx}
LITECOINQT=${BITCOINQT:-$BINDIR/qt/nutucoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
NTCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
